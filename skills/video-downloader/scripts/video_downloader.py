...
def parse_args():
    ...
    parser.add_argument('--count', type=int, default=3, help='Number of videos to download (default: 3)')
    ...
    parser.add_argument('--quality', choices=['4k', '1080p', '720p', '480p'],
                        default='720p', help='Video quality (default: 720p)')
    parser.add_argument('--platform', help='Specify platform (bilibili/youtube, etc.)')
    parser.add_argument('--output-dir', default=os.path.expanduser('~/Downloads/videos'),
                        help='Output directory (default: ~/Downloads/videos)')
    parser.add_argument('--urls', nargs='+', help='List of video URLs')
    parser.add_argument('--no-transcode', action='store_true', help='Disable transcoding')
    parser.add_argument('--force-transcode', action='store_true', help='Force transcoding')
...
def main():
    ...
    if args.urls:
        ...
        print("   Use browser automation tools (headless mode recommended) to search and extract URLs")
        ...
        print("   Open the above URL, copy video URLs, and re-run the command with --urls parameter")
        print()
    ...
    for i, url_info in enumerate(urls_to_process[:args.count], 1):
        ...
        if file_path:
            downloaded_files.append((i, file_path))
        print()
    ...
    if not args.no_transcode:
        ...
        for index, input_file in downloaded_files:
            base_name = os.path.basename(input_file)
            ...
            if needs:
                output_file = transcode_video(input_file, output_dir, args.quality, index)
                if output_file:
                    transcoded_files.append((index, output_file))
            else:
                # Already compatible, copy directly to ppt_compatible subdirectory
                ppt_dir = os.path.join(output_dir, 'ppt_compatible')
                os.makedirs(ppt_dir, exist_ok=True)
                ...
                name_without_ext = os.path.splitext(base_name)[0]
                name_clean = re.sub(r'^\d+_', '', name_without_ext)
                # Generate new filename: {index:02d}_{clean_name}.mp4
                output_file = os.path.join(ppt_dir, f"{index:02d}_{name_clean}.mp4")
                ...
                copied_files.append((index, output_file))
        ...
        if all_ppt_files:
            ...
            for index, file_path in all_ppt_files:
                ...
                if info:
                    ...
                    status = "Transcoded" if index <= len(transcoded_files) else "Compatible"
                    print(f"  Success [{status}] {os.path.basename(file_path)}")
                    print(f"    Codec: {codec}, Resolution: {width}x{height}, Frame rate: {fps}")
                print()
        ...
        print()
        print("Tip: When inserting into PPT, use files from the ppt_compatible directory")
